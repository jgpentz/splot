...
export default function FileOptions({ fileData, setFileData, sparams, setSparams, fname, snames }: FileOptionsProps) {
    ...
    const handleBlur = () => {
        ...
        // Updated filename stored in fileData
        setFileData((prevFileData) => {
            const updatedFileData = { ...prevFileData };
            if (fname !== text) {
                Object.defineProperty(updatedFileData, text,
                    Object.getOwnPropertyDescriptor(updatedFileData, fname) as any
                );
                delete updatedFileData[fname]
            }
        ...
        // Updated filename stored in sparams
    ...
    const deleteFile = (() => {
        setFileData(prevFileData => {
            const { [fname]: _, ...newFileData } = prevFileData;
            return newFileData;
        })
    ...
    const items = snames.map((sname, index) => (
        <Grid key={`${sname}-${index}`} className={classes.Sparams} columns={20}>
            ...
            {/* Select line style */}
            <Grid.Col span={3}>
                <ActionIcon disabled className={classes.Icon} variant="transparent" onClick={() => setVisible((o) => !o)}>
